...
CREATE VIEW v_dpcidade(dpto_nome,cid_nome) AS SELECT d.nome AS dpto_nome,c.nome AS cid_nome FROM tbl_cidades c
 JOIN tbl_departamentos d ON c.cod_cidade = d.cod_cidade;
...
CREATE VIEW v_dpcidadehouston AS SELECT dpto_nome FROM v_dpcidade WHERE cid_nome = 'Houston';
...
CREATE VIEW v_opsalario (soma,media) AS SELECT SUM(salario) AS soma,AVG(salario) AS media from tbl_empregados;
...
CREATE VIEW vw_empregados_salarial (nome,salario) AS SELECT  nome,salario FROM tbl_empregados WHERE salario >5500;
...
-- listar os departamentos e as quantidades por orderm decrescente de quantidade.
 CREATE VIEW v_departemp (nomedpto,qtd) AS SELECT d.nome AS nomedpto, COUNT (e.nome) AS qtd 
 FROM tbl_departamentos d JOIN tbl_empregados e ON d.cod_departamento = e.cod_departamento GROUP BY d.nome ORDER BY qtd DESC;
...
SELECT d.nome AS nomedpto
FROM tbl_departamentos d
JOIN tbl_empregados e ON d.cod_departamento = e.cod_departamento
JOIN tbl_projetos p ON d.cod_departamento = p.cod_departamento
GROUP BY d.nome
HAVING COUNT(p.nome) > COUNT(e.nome);