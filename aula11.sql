...
language plpgsql AS $$
BEGIN
UPDATE tbl_departamentos SET nome = novonome
...
language plpgsql AS $$
BEGIN 
CREATE TABLE tblcopia_cidades AS TABLE tbl_cidades;
END $$;
...
language plpgsql AS $$
BEGIN 
DELETE FROM tbl_projetos WHERE codproj = cod_projeto;
END $$;
...
CREATE OR REPLACE FUNCTION proc_projeto_arquivado(
    ...
    language plpgsql AS $$
    BEGIN
    CREATE TABLE IF NOT EXISTS tbl_projetos_arquivados (cod_projeto integer primary key, nome text);
    INSERT INTO tbl_projetos_arquivados (cod_projeto,nome) SELECT (cod_projeto,nome) FROM tbl_projetos WHERE codigo = cod_projeto;
    DELETE FROM tbl_projetos WHERE codigo = cod_projeto;
    END $$;