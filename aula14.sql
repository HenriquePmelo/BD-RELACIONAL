...
CREATE OR REPLACE TRIGGER tr_attproj
AFTER DELETE ON tbl_departamentos
FOR EACH ROW
EXECUTE FUNCTION fc_atualizaprojeto;

CREATE OR REPLACE FUNCTION fc_atualizaprojeto
RETURNS TRIGGER
language plpgsql AS $$
BEGIN 
SELECT COUNT (qtde_projetos) FROM tbl_departamentos; 
RETURN OLD;
END $$; 

--2 crie uma trigger que atualiza a quantidade de projetos da tbl_departamentos caso um novo projeto for adicionado
CREATE OR REPLACE TRIGGER tr_addproj
AFTER UPDATE ON tbl_departamentos
FOR EACH ROW
EXECUTE FUNCTION fc_addprojeto;

CREATE OR REPLACE FUNCTION fc_addprojeto
RETURNS TRIGGER
language plpgsql AS $$
BEGIN
SELECT COUNT (qtde_projetos) FROM tbl_departamentos;
