...
BEGIN 
maiorsal := 0;
SELECT nome FROM tbl_empregados
WHERE salario > maiorsal
...
media float;
BEGIN
SELECT AVG (salario) INTO media from tbl_empregados;
RETURN media;
END $$;
...
RETURNS TABLE (salario float , saldez float , salreduz float)
language plpgsql AS $$
DECLARE
salemp float;
saldez float;
salreduz float;
BEGIN
SELECT salario INTO salemp from tbl_empregados WHERE cod_empregado = codigoemp;
saldez := salemp*1.1;
salreduz := salemp*0.85;
RETURN QUERY SELECT salemp, saldez , salreduz;
END $$;
...
resultado integer;
BEGIN 
SELECT COUNT (cod_departamento) INTO codigodpt from tbl_projetos 
WHERE cod_departamento = codigodpt;
RETURN resultado;
END $$;
